...
static GtkWidget* create_terminal_tab(TerminalData **data_ptr) {
    ...
    *data_ptr = data;
    ...
    vte_terminal_spawn_async(VTE_TERMINAL(terminal), VTE_PTY_DEFAULT, cwd, argv, NULL, G_SPAWN_DEFAULT, NULL, NULL, NULL, -1, NULL, on_terminal_spawn_success, data);
...
static void tentang_terminal(GtkButton *button, gpointer user_data) {
    GtkWidget *dialog = gtk_dialog_new_with_buttons(
        "_Tutup",
        GTK_WINDOW(main_window),
        GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT,
        "_Tutup", GTK_RESPONSE_CLOSE,
        NULL
    );
    ...
    if (page) {
        ...
        if (data && data->terminal) {
            gtk_widget_grab_focus(GTK_WIDGET(data->terminal));
        }
    }
}
...
static void activate(GtkApplication *app, gpointer user_data) {
    ...
    gtk_header_bar_set_title(GTK_HEADER_BAR(header), "indoterm");
    gtk_header_bar_set_show_close_button(GTK_HEADER_BAR(header), TRUE);
    ...
    GtkWidget *menu_icon = gtk_image_new_from_icon_name("gtk-about", GTK_ICON_SIZE_BUTTON);
    ...
    g_signal_connect(menu_btn, "clicked", G_CALLBACK(tentang_terminal), NULL);
